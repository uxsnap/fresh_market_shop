...
func (r *BasePgRepository) DeleteCascade(ctx context.Context, row Row, condition sq.Sqlizer) error {
	stmt, args, err := sq.Delete(row.Table()).PlaceholderFormat(sq.Dollar).Where(condition).Suffix("CASCADE").ToSql()
	if err != nil {
		return err
	}
	_, err = r.DB().Exec(ctx, stmt, args...)
